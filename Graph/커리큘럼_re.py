...
queue = deque()
result = course_time.copy()  # 결과값. 누적 수강 시간
...
while queue:
    ...
    for destination in graph[now]:  # 연결된 간선 없애기
        ...
        # max(현재까지의 수강시간 + 목표 수강시간)
        result[destination] = max(result[destination], result[now] + course_time[destination])

        if indegree[destination] == 0:  # 0 되면 큐에 넣기
